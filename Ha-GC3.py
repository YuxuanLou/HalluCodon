...
class MLMDataCollator:
    ...
    def _create_mask_pattern(self, seq_length, device):
        ...
        indices_random = torch.bernoulli(random_prob_matrix,
        ...
        self.random_cache[seq_length] = {"mask": indices_mask.cpu(), "random": indices_random.cpu(),
...
class GradientOptimizer:
    ...
    def calculate_naturalness_with_mlm(self, cds_sequences, nat_protein_emb):
        ...
        for i in range(0, len(cds_sequences), self.batch_size):
            ...
            cds_encoding = self.cds_tokenizer(
                ...
                return_tensors='pt')
            batch_samples = [{'cds_input_ids': cds_encoding['input_ids'][j].squeeze(),
            ...
            for seed_idx, mlm_collator in enumerate(mlm_collators):
                ...
                with torch.no_grad():
                    ...
                    outputs = self.naturality_model(cds_input_ids=batch['cds_input_ids'],
        ...
        for seq_idx in range(num_sequences):
            scores_for_seq = [all_scores_by_seed[s][seq_idx] for s in range(n_seeds) if
                              seq_idx < len(all_scores_by_seed[s])]
            if scores_for_seq:
    ...
    def _apply_random_synonymous_mutations(self, cds_sequence, num_mutations=1):
        ...
        mutable_indices = [i for i, codon in enumerate(codons) if
                           codon_table.get(codon, '*') != '*' and len(
                               synonymous_codons.get(codon_table.get(codon), [])) > 1]
        ...
        while True:
            ...
            for _ in range(self.iterations):
                ...
                if not best_proposals_by_pos:
                    ...
                    mutated_codons_list = [current_sequence_for_generation[i:i + 3] for i in
                                           range(0, len(current_sequence_for_generation), 3)]
                    ...
                    if self.use_reversibility_check:
                        ...
                        if best_second_pass_proposals:
                            sorted_second_pass_proposals = sorted(best_second_pass_proposals.items(),
            ...
            if new_sequences_to_evaluate:
                ...
                for seq, expr, nat in zip(new_sequences_to_evaluate, expression_scores, naturality_scores):
                    ...
                    origin_iteration = all_sequences_and_origins.get(seq, total_iterations_run)
                    log_records.append({
